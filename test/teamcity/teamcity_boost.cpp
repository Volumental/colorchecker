...
#else
    virtual void log_entry_context(std::ostream&, boost::unit_test::log_level l, boost::unit_test::const_string);
    virtual void entry_context_finish(std::ostream&, boost::unit_test::log_level l);
#endif
};
    ...
    if (tu.p_type == TUT_CASE_IDENTIFIER) {
        if(!tr.passed()) {
            if(tr.p_skipped) {
                ...
                LOG(ERROR) << "\n" << komb::stacktraceAndErrorContext(1);
                LOG(ERROR) << "TeamCity: Test Failed: "
                    << tu.p_name << " " << currentDetails << " " << flowId;
            }
        }
...
void TeamcityBoostLogFormatter::log_exception(std::ostream &out, boost::unit_test::log_checkpoint_data const &, boost::unit_test::const_string explanation) {
    ...
    LOG(ERROR) << "\n" << komb::stacktraceAndErrorContext(1);
    LOG(ERROR) << "TeamCity: exception thrown:\n" << currentDetails;
}
